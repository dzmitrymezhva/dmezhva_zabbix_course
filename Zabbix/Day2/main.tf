...
resource "google_compute_instance" "vm2" {
  ...
  metadata_startup_script = templatefile("tomcat.sh", {staticIP = "${var.static_ip_vm1_address}"})
}
...
resource "google_compute_firewall" "firewall_tomcat" {
  name                    = "${var.name}-firewall-tomcat"
  network                 = google_compute_network.vpc_zabbix.name
  source_ranges           = var.firewall_tomcat_source_ranges

  allow {
    protocol              = var.firewall_tomcat_protocol
    ports                 = var.firewall_tomcat_ports
  }
